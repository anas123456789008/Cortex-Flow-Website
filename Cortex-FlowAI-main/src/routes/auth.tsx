...
function AuthPage() {
  ...
  async function googleSignIn() {
    ...
    // On success Supabase redirects the browser to Google itself, so this
    // line only runs if something went wrong before that redirect happened.
    if (error) {
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="rounded-2xl border border-border bg-glass p-7 backdrop-blur-2xl shadow-[0_30px_80px_-30px_rgba(139,92,246,0.4)]">
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            ...
            <Field icon={Mail} type="email" placeholder="Email address" value={email} onChange={setEmail} autoComplete="new-email" required />
            {mode === "forgot" && (
